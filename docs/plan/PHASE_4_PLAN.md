...
### Execution Checklist
- [ ] `backend/models/models.py`: add `Purchase` model — fields: `id, created_by, supplier, status, total_cost, created_at, approved_by, approved_at`
- [ ] `backend/models/models.py`: add `PurchaseItem` model — fields: `id, purchase_id, product_id, quantity, cost_price`
- [ ] `database/schema.sql`: add `purchases` and `purchase_items` tables matching ORM models
